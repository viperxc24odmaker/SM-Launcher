...
export type AccountKind = 'offline' | 'microsoft' | 'elyby' | 'sklauncher' | 'yggdrasil';
export interface LauncherCape { id?: string; name?: string; url?: string; source: 'minecraft' | 'sklauncher' | 'elyby' | 'unknown' }
export interface LauncherSkin { id?: string; url?: string; variant?: string; source: 'minecraft' | 'sklauncher' | 'elyby' | 'unknown' }
export interface LauncherAccount { id:string; kind:AccountKind; username:string; uuid:string; accessToken:string; clientToken:string; profile?:Record<string,unknown>; server?:string; skin?:LauncherSkin; cape?:LauncherCape }
...
export class AccountManager{
 ...
 async startMicrosoftLogin(){const win=BrowserWindow.getAllWindows()[0];const cacheDir=path.join(app.getPath('userData'),'auth-cache');const flow=new Authflow(`sm-launcher-${Date.now()}`,cacheDir,undefined,(code:any)=>win?.webContents.send('auth:microsoft-code',code));const result=await flow.getMinecraftJavaToken({fetchProfile:true});if(!result.profile)throw new Error('Microsoft account has no Minecraft Java profile.');const profile:any=result.profile;const account:LauncherAccount={id:randomUUID(),kind:'microsoft',username:profile.name,uuid:profile.id,accessToken:result.token,clientToken:randomUUID(),profile};const s=read();s.accounts=s.accounts.filter(a=>a.username!==account.username);s.accounts.push(account);s.activeId=account.id;write(s);return this.publicAccount(account);}
 async addElyBy(input:{username:string;password:string;totp?:string}){const clientToken=randomUUID();const password=input.totp?`${input.password}:${input.totp}`:input.password;const response=await fetch('https://authserver.ely.by/auth/authenticate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:input.username,password,clientToken,requestUser:true})});const data:any=await response.json();if(!response.ok)throw new Error(data.errorMessage||data.error||'Ely.by authentication failed.');const profile=data.selectedProfile||data.user?.selectedProfile;if(!profile)throw new Error('Ely.by did not return a Minecraft profile.');const account:LauncherAccount={id:randomUUID(),kind:'elyby',username:profile.name,uuid:profile.id,accessToken:data.accessToken,clientToken,profile};const s=read();s.accounts=s.accounts.filter(a=>a.username!==account.username);s.accounts.push(account);s.activeId=account.id;write(s);this.refreshCosmetics(account);return this.publicAccount(account);}
 async refreshCosmetics(account:LauncherAccount){
   // Microsoft cosmetics are obtained from the official Minecraft profile response when present.
   // SKlauncher cosmetics are a separate service and require an SKlauncher account/session.
   // We intentionally do not scrape or impersonate private SKlauncher endpoints.
   const profile:any=account.profile;
   const textures=profile?.skins||profile?.capes;
   if(Array.isArray(textures)){
     const skin=textures.find((x:any)=>x.state==='ACTIVE'||x.state==='active');
     const cape=Array.isArray(profile?.capes)?profile.capes.find((x:any)=>x.state==='ACTIVE'||x.state==='active'):undefined;
     if(skin)account.skin={id:skin.id,url:skin.url,variant:skin.variant,source:'minecraft'};
     if(cape)account.cape={id:cape.id,name:cape.alias,url:cape.url,source:'minecraft'};
   }
   const s=read();const index=s.accounts.findIndex(a=>a.id===account.id);if(index>=0){s.accounts[index]=account;write(s);}return this.publicAccount(account);
 }
