...
#[tauri::command]
pub async fn install_minecraft(version:String,gameDir:String)->Result<String,String>{
 ...
 let root=PathBuf::from(gameDir);fs::create_dir_all(&root).map_err(|e|e.to_string())?;let vd=root.join("versions").join(&version);fs::create_dir_all(&vd).map_err(|e|e.to_string())?;
